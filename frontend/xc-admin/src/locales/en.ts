export default {
  app: {
    title: 'XC Admin',
    shortTitle: 'XC Admin'
  },
  common: {
    account: 'Account',
    password: 'Password',
    login: 'Sign in',
    logout: 'Sign out',
    refresh: 'Refresh',
    language: 'Language',
    profile: 'Admin',
    noPermission: 'No permission',
    comingSoon: 'Coming soon',
    empty: '-',
    openMenu: 'Expand menu',
    closeMenu: 'Collapse menu',
    search: 'Search',
    fullscreen: 'Fullscreen',
    notifications: 'Notifications',
    settings: 'Settings',
    tabActions: 'Tab actions',
    refreshCurrent: 'Refresh current',
    closeCurrent: 'Close current',
    closeLeft: 'Close left',
    closeRight: 'Close right',
    closeOthers: 'Close others',
    closeAll: 'Close all'
  },
  login: {
    title: 'XC Admin',
    subtitle: 'Operations console for content, memberships, and learning data.',
    accountPlaceholder: 'Admin username',
    passwordPlaceholder: 'Password',
    accountRequired: 'Please enter account',
    passwordRequired: 'Please enter password',
    success: 'Signed in'
  },
  menus: {
    dashboard: 'Dashboard',
    users: 'Users',
    memberships: 'Memberships',
    content: 'Content',
    reports: 'Reports',
    system: 'System'
  },
  dashboard: {
    title: 'Dashboard',
    subtitle: 'Overview of users, memberships, content, and learning activity.',
    groups: {
      userMembership: 'Users & memberships',
      orders: 'Orders & payments',
      contentAssets: 'Content assets'
    },
    userCount: 'Users',
    activeUserCount: 'Active users',
    disabledUserCount: 'Disabled users',
    trialUserCount: 'Trial users',
    todayNewUserCount: 'New today',
    activeMembershipCount: 'Active members',
    activePlanCount: 'Active plans',
    todayOrderCount: 'Orders today',
    pendingOrderCount: 'Pending orders',
    paidOrderCount: 'Paid orders',
    todayPaidAmount: 'Paid today',
    vocabListCount: 'Vocab lists',
    inactiveVocabListCount: 'Inactive vocab lists',
    vocabItemCount: 'Vocab items',
    exerciseSetCount: 'Exercise sets',
    inactiveExerciseSetCount: 'Inactive exercise sets',
    sentenceExerciseCount: 'Sentence exercises',
    videoMaterialCount: 'Dialogue materials',
    inactiveVideoMaterialCount: 'Inactive materials',
    dialogueLineCount: 'Dialogue lines'
  },
  content: {
    title: 'Content',
    subtitle: 'Maintain vocab, exercises, and media assets for stable student-facing content.',
    tabs: {
      vocabLists: 'Vocab lists',
      vocabItems: 'Vocab items',
      mediaAssets: 'Media assets',
      exerciseSets: 'Exercise sets',
      sentenceExercises: 'Sentence exercises',
      videoMaterials: 'Dialogue materials',
      dialogueLines: 'Dialogue lines',
      lineVocab: 'Line vocab'
    },
    parentReadonly: {
      vocabItems: 'The parent vocab list is inactive. Vocab items are read-only.',
      sentenceExercises: 'The parent exercise set is inactive. Sentence exercises are read-only.',
      dialogueLines: 'The parent dialogue material is inactive. Dialogue lines are read-only.',
      lineVocab: 'The parent dialogue material is inactive. Vocab analysis is read-only.',
      submitDisabled: 'The parent is inactive. Child records are read-only.',
      importDisabled: 'The parent is inactive. Templates and CSV imports are disabled for child records.'
    },
    listKeyword: 'Search name, level, or description',
    itemKeyword: 'Search hanzi, pinyin, English or Russian meaning',
    mediaKeyword: 'Search original filename or resource URL',
    setKeyword: 'Search set title or level',
    exerciseKeyword: 'Search answer, pinyin, English or Russian prompt',
    materialKeyword: 'Search material title or description',
    lineKeyword: 'Search Chinese, pinyin, English or Russian text',
    lineVocabKeyword: 'Search word, pinyin, meaning, or explanation',
    typeFilter: 'List type',
    exerciseTypeFilter: 'Exercise type',
    materialTypeFilter: 'Material type',
    materialFilter: 'Dialogue material',
    lineFilter: 'Dialogue line',
    levelFilter: 'Level',
    statusFilter: 'Status',
    listFilter: 'Vocab list',
    mediaTypeFilter: 'Media type',
    languageFilter: 'Language',
    audioFilter: 'Audio status',
    coverFilter: 'Cover status',
    reset: 'Reset',
    listTitle: 'Vocab lists',
    itemTitle: 'Vocab items',
    mediaTitle: 'Media assets',
    setTitle: 'Exercise sets',
    exerciseTitle: 'Sentence exercises',
    materialTitle: 'Dialogue materials',
    lineTitle: 'Dialogue lines',
    lineVocabTitle: 'Dialogue line vocab',
    total: '{total} records',
    emptyLists: 'No vocab lists',
    emptyItems: 'No vocab items',
    emptyMedia: 'No media assets',
    emptySets: 'No exercise sets',
    emptyExercises: 'No sentence exercises',
    emptyMaterials: 'No dialogue materials',
    emptyLines: 'No dialogue lines',
    emptyLineVocab: 'No vocab analysis',
    itemSummary: 'Active {active} / inactive {inactive}',
    exerciseSummary: 'Active {active} / inactive {inactive}',
    childSummary: '{count} children',
    createListTitle: 'Create vocab list',
    editListTitle: 'Edit vocab list',
    createItemTitle: 'Create vocab item',
    editItemTitle: 'Edit vocab item',
    uploadMediaTitle: 'Upload media asset',
    createSetTitle: 'Create exercise set',
    editSetTitle: 'Edit exercise set',
    createExerciseTitle: 'Create sentence exercise',
    editExerciseTitle: 'Edit sentence exercise',
    createMaterialTitle: 'Create dialogue material',
    editMaterialTitle: 'Edit dialogue material',
    createLineTitle: 'Create dialogue line',
    editLineTitle: 'Edit dialogue line',
    createLineVocabTitle: 'Create vocab analysis',
    editLineVocabTitle: 'Edit vocab analysis',
    importCsvTitle: 'Import content CSV',
    uploadDropText: 'Drop a file here or click to choose',
    importDropText: 'Drop a CSV file here or click to choose',
    importCsvHint: 'New templates use business-friendly column names. Record ID may be empty to create records, or filled to update records. Audio/cover asset IDs may be empty and bound later. Child content is imported into the selected parent above. Use UTF-8 CSV.',
    importResultTitle: 'CSV import result',
    importResult: 'Imported {success} / {requested} rows',
    importFailed: 'CSV import failed',
    importErrorFixHint: 'Fix the CSV by the row number and field name below, then import again. If some rows already succeeded, re-import only the failed rows to avoid duplicate creates.',
    importRequestErrorHint: 'The import did not run. Download the latest template, keep required headers unchanged, save as UTF-8 CSV, then try again.',
    importTraceId: 'Trace ID: {traceId}',
    importTemplateUnavailable: 'Media assets do not use CSV templates. Upload media files from the media tab.',
    templateDownloaded: 'Template download started',
    templateDownloadFailed: 'Template download failed',
    loadChildrenFailed: 'Failed to load children',
    statusDialogTitle: 'Update content status',
    statusReasonPlaceholder: 'Optional reason',
    reasonTooLong: 'Reason must be 1000 characters or less',
    cancel: 'Cancel',
    submit: 'Submit',
    saved: 'Saved',
    listTypes: {
      HSK: 'HSK',
      YCT: 'YCT',
      category: 'Category',
      professional: 'Professional',
      custom: 'Custom'
    },
    status: {
      all: 'All',
      active: 'Active',
      inactive: 'Inactive'
    },
    mediaTypes: {
      audio: 'Audio',
      image: 'Image',
      video: 'Video'
    },
    languages: {
      zh: 'Chinese',
      ru: 'Russian',
      en: 'English'
    },
    assetFilters: {
      withAudio: 'Has audio',
      missingAudio: 'Missing audio',
      withCover: 'Has cover',
      missingCover: 'Missing cover'
    },
    importTypes: {
      'vocab-lists': 'Vocab lists',
      'vocab-items': 'Vocab items',
      'exercise-sets': 'Exercise sets',
      'sentence-exercises': 'Sentence exercises',
      'video-materials': 'Dialogue materials',
      'dialogue-lines': 'Dialogue lines',
      'dialogue-line-vocab': 'Line vocab'
    },
    importContext: {
      required: 'Select {target} before downloading a template or importing CSV',
      targets: {
        vocabList: 'parent vocab list',
        exerciseSet: 'parent exercise set',
        material: 'parent dialogue material',
        dialogueLine: 'parent dialogue line'
      },
      placeholders: {
        vocabList: 'Select the vocab list for this import',
        exerciseSet: 'Select the exercise set for this import',
        material: 'Select the dialogue material for this import',
        dialogueLine: 'Select the dialogue line for this import'
      }
    },
    bulkBind: {
      hint: 'Enter one pair per line: content ID, media asset ID. Commas, Chinese commas, spaces, and tabs are supported.',
      placeholder: 'Example:\n101, 3001\n102, 3002\n103, 3003',
      emptyRows: 'Enter at least one binding row',
      invalidTitle: 'Invalid binding format',
      lineFormatError: 'Line {line} is invalid. Enter two positive integer IDs.',
      resultTitle: 'Bulk binding result',
      result: 'Bound {success} / {requested} records',
      targets: {
        itemAudio: 'Bulk bind vocab audio',
        exerciseAudio: 'Bulk bind exercise audio',
        lineAudio: 'Bulk bind line audio',
        materialCover: 'Bulk bind material covers'
      }
    },
    exerciseTypes: {
      audio_order: 'Audio word order',
      audio_dictation: 'Audio dictation',
      pinyin_dictation: 'Pinyin dictation',
      translation_order: 'Pinyin word order'
    },
    materialTypes: {
      drama: 'Drama',
      short_video: 'Short video',
      cartoon: 'Cartoon'
    },
    columns: {
      vocabList: 'Vocab list',
      parent: 'Parent',
      type: 'Type',
      level: 'Level',
      items: 'Items',
      sortOrder: 'Sort',
      status: 'Status',
      updatedAt: 'Updated at',
      actions: 'Actions',
      word: 'Word',
      meaningEn: 'English',
      meaningRu: 'Russian',
      media: 'Original file / resource',
      mediaType: 'Type',
      language: 'Language',
      duration: 'Duration',
      createdAt: 'Created at',
      exerciseSet: 'Exercise set',
      exercises: 'Exercises',
      answer: 'Answer',
      prompt: 'Prompt',
      audio: 'Audio',
      options: 'Options',
      material: 'Material',
      cover: 'Cover',
      lines: 'Lines',
      lineNo: 'Line',
      timeRange: 'Time',
      line: 'Dialogue line',
      linkedVocab: 'Linked vocab',
      explanation: 'Explanation'
    },
    fields: {
      name: 'Name',
      listNamePlaceholder: 'Example: HSK1 basic vocab',
      listType: 'Type',
      parentVocabList: 'Parent vocab list',
      parentExerciseSet: 'Parent exercise set',
      parentMaterial: 'Parent dialogue material',
      level: 'Level',
      sortOrder: 'Sort',
      status: 'Status',
      description: 'Description',
      vocabList: 'Vocab list',
      hanzi: 'Hanzi',
      pinyin: 'Pinyin',
      meaningEn: 'English meaning',
      meaningRu: 'Russian meaning',
      exampleSentence: 'Example sentence',
      audioAsset: 'Audio asset',
      strokeOrderAsset: 'Stroke-order image',
      mediaType: 'Media type',
      language: 'Language',
      durationMs: 'Duration (ms)',
      file: 'File',
      title: 'Title',
      setTitlePlaceholder: 'Example: HSK1 audio word order',
      exerciseType: 'Exercise type',
      exerciseSet: 'Exercise set',
      hanziAnswer: 'Chinese answer',
      pinyinPrompt: 'Pinyin prompt',
      translationEn: 'English prompt',
      translationRu: 'Russian prompt',
      audioZhAsset: 'Chinese audio',
      explanation: 'Explanation',
      wordOptions: 'Word options',
      wordOptionsPlaceholder: 'One option per line; line order is saved as the correct order',
      materialTitle: 'Material title',
      materialTitlePlaceholder: 'Example: Daily greetings clip',
      materialType: 'Material type',
      coverAsset: 'Cover image',
      material: 'Dialogue material',
      lineNo: 'Line number',
      hanziText: 'Chinese line',
      pinyinText: 'Pinyin',
      startMs: 'Start time (ms)',
      endMs: 'End time (ms)',
      dialogueLine: 'Dialogue line',
      vocabItem: 'Linked vocab',
      wordText: 'Word',
      lineVocabExplanation: 'Extra explanation',
      importType: 'Import type'
    },
    validation: {
      nameRequired: 'Please enter name',
      nameTooLong: 'Name must be 100 characters or less',
      typeRequired: 'Please select type',
      sortRequired: 'Please enter sort value',
      listRequired: 'Please select vocab list',
      hanziRequired: 'Please enter hanzi',
      hanziTooLong: 'Hanzi must be 100 characters or less',
      mediaTypeRequired: 'Please select media type',
      fileRequired: 'Please choose a file',
      fileLimit: 'Only one file can be uploaded at a time',
      titleRequired: 'Please enter title',
      exerciseSetRequired: 'Please select exercise set',
      answerRequired: 'Please enter Chinese answer',
      materialRequired: 'Please select dialogue material',
      lineRequired: 'Please select dialogue line',
      lineNoRequired: 'Please enter line number',
      lineTextRequired: 'Please enter Chinese line',
      wordRequired: 'Please enter word',
      deleteConfirm: 'Delete this vocab analysis item?',
      mediaDeleteConfirm: 'Permanently delete this media asset? The stored object will also be removed and cannot be recovered.'
    },
    actions: {
      createList: 'Create list',
      createItem: 'Create item',
      uploadMedia: 'Upload media',
      uploadAudio: 'Upload audio',
      uploadImage: 'Upload image',
      uploadStrokeImage: 'Upload stroke image',
      createSet: 'Create set',
      createExercise: 'Create exercise',
      createMaterial: 'Create material',
      createLine: 'Create line',
      createLineVocab: 'Create analysis',
      edit: 'Edit',
      delete: 'Delete',
      items: 'Items',
      exercises: 'Exercises',
      lines: 'Lines',
      enable: 'Enable',
      disable: 'Disable',
      open: 'Open',
      bulkBindAudio: 'Bulk bind audio',
      bulkBindCover: 'Bulk bind covers',
      batchImportItems: 'Import vocab CSV',
      batchImportExercises: 'Import sentence CSV',
      batchUpdateVocabLists: 'Batch edit lists',
      batchUpdateExerciseSets: 'Batch edit sets',
      batchEnable: 'Batch enable',
      batchDisable: 'Batch disable',
      downloadTemplate: 'Download template',
      importCsv: 'CSV import',
      importCsvWithTemplate: 'CSV import / template'
    },
    batchStatus: {
      selected: '{count} selected',
      emptySelection: 'Select content records first',
      confirmTitle: '{action} {target}',
      reasonPlaceholder: 'Optional batch status reason',
      resultTitle: 'Batch status result',
      result: '{action} succeeded for {success} / {requested} records',
      targets: {
        lists: 'vocab lists',
        items: 'vocab items',
        media: 'media assets',
        sets: 'exercise sets',
        exercises: 'sentence exercises',
        materials: 'dialogue materials'
      }
    },
    batchAssignments: {
      titleItems: 'Batch edit assigned lists',
      titleExercises: 'Batch edit assigned sets',
      fieldVocabLists: 'Assigned vocab lists',
      fieldExerciseSets: 'Assigned exercise sets',
      replaceHint: 'Submitting will replace all existing assignments for the selected records.',
      emptyTarget: 'Select at least one target category',
      removeSuccess: 'Assignment removed',
      removeFailed: 'Failed to remove assignment. Please try again later.',
      resultTitle: 'Batch assignment result',
      result: 'Batch assignment succeeded for {success} / {requested} records',
      placeholders: {
        vocabLists: 'Select one or more vocab lists',
        exerciseSets: 'Select one or more exercise sets'
      }
    }
  },
  memberships: {
    title: 'Memberships',
    subtitle: 'Configure membership plans and review payment orders, provider status, and callback audit records.',
    tabs: {
      plans: 'Plans',
      orders: 'Orders',
      notifications: 'Callbacks'
    },
    planKeyword: 'Search plan name',
    orderKeyword: 'Order no, trade no, user email or nickname',
    notificationKeyword: 'Order no, trade no, user, or result message',
    statusFilter: 'Plan status',
    orderStatusFilter: 'Order status',
    notificationStatusFilter: 'Process status',
    providerFilter: 'Provider',
    clientFilter: 'Client',
    signatureFilter: 'Signature',
    resultCodeFilter: 'Result code',
    exceptionFilter: 'Exception type',
    exceptionQuickTitle: 'Quick views',
    exceptionViewWithCount: '{label} ({count})',
    reset: 'Reset',
    total: '{total} records',
    planListTitle: 'Plans',
    orderListTitle: 'Orders',
    notificationListTitle: 'Callbacks',
    orderCallbacksTitle: 'Order callback chain',
    orderOperationLogsTitle: 'Handling records',
    emptyPlans: 'No plans',
    emptyOrders: 'No orders',
    emptyNotifications: 'No callback records',
    emptyOrderCallbacks: 'No callback records for this order',
    emptyOrderOperationLogs: 'No handling records for this order',
    unnamed: 'No nickname',
    createPlanTitle: 'Create plan',
    editPlanTitle: 'Edit plan',
    createOfflinePaymentTitle: 'Register offline payment',
    orderDetailTitle: 'Order details',
    notificationDetailTitle: 'Callback details',
    markFailedTitle: 'Mark order failed',
    statusDialogTitle: 'Update plan status',
    statusReasonPlaceholder: 'Optional reason',
    failReasonPlaceholder: 'Enter why this order cannot be completed',
    providerTradeNo: 'Provider trade no',
    paymentUrl: 'Payment URL',
    paidAt: 'Paid at',
    receivedAt: 'Received at',
    resultCode: 'Result code',
    resultMessage: 'Result message',
    signatureValid: 'Signature',
    notifyPayload: 'Raw callback payload',
    latestCallback: 'Latest callback',
    failureReason: 'Failure reason',
    processStatusLabel: 'Process status',
    orderStatusLabel: 'Order status',
    cancel: 'Cancel',
    submit: 'Submit',
    saved: 'Saved',
    orderMarkedFailed: 'Order marked as failed',
    offlineOrderCreated: 'Offline payment registered and membership activated',
    days: '{days} days',
    columns: {
      planName: 'Plan',
      duration: 'Duration',
      price: 'Price',
      status: 'Status',
      updatedAt: 'Updated at',
      actions: 'Actions',
      orderNo: 'Order no',
      user: 'User',
      amount: 'Amount',
      provider: 'Provider',
      client: 'Client',
      exception: 'Exception',
      action: 'Action',
      admin: 'Operator',
      reason: 'Reason',
      createdAt: 'Created at'
    },
    fields: {
      name: 'Plan name',
      namePlaceholder: 'Example: Monthly member',
      durationUnit: 'Duration unit',
      durationValue: 'Duration value',
      price: 'Price',
      currency: 'Currency',
      status: 'Status',
      userKeyword: 'User',
      userKeywordPlaceholder: 'Enter user ID, email, or nickname',
      offlinePlan: 'Membership plan',
      offlinePlanPlaceholder: 'Select a plan',
      amount: 'Received amount',
      paidAt: 'Paid at',
      paidAtPlaceholder: 'Defaults to now',
      offlineTradeNo: 'Offline reference',
      offlineTradeNoPlaceholder: 'Receipt no, transfer reference, etc.',
      remark: 'Remark',
      remarkPlaceholder: 'Describe the offline payment, such as method, confirmer, or business context'
    },
    validation: {
      nameRequired: 'Please enter plan name',
      nameTooLong: 'Plan name must be 100 characters or less',
      durationUnitRequired: 'Please select duration unit',
      durationValueRequired: 'Please enter duration value',
      priceRequired: 'Please enter price',
      currencyRequired: 'Please enter currency',
      userRequired: 'Please enter user ID, email, or nickname',
      userTooLong: 'User value must be 128 characters or less',
      planRequired: 'Please select a plan',
      amountRequired: 'Please enter received amount',
      remarkRequired: 'Please enter a remark',
      failReasonRequired: 'Please enter a handling reason',
      reasonTooLong: 'Reason must be 1000 characters or less'
    },
    durationUnits: {
      day: 'Day',
      month: 'Month',
      custom: 'Custom'
    },
    planStatus: {
      active: 'Active',
      inactive: 'Inactive'
    },
    orderStatus: {
      pending: 'Pending',
      paid: 'Paid',
      failed: 'Failed',
      refunded: 'Refunded'
    },
    exceptionViews: {
      allOrders: 'All orders',
      allExceptions: 'All exceptions'
    },
    orderExceptions: {
      all: 'All exceptions',
      pending_timeout: 'Pending timeout',
      callback_failed: 'Callback failed',
      amount_mismatch: 'Amount mismatch',
      provider_mismatch: 'Provider mismatch',
      membership_missing: 'Membership missing'
    },
    operationActions: {
      offlinePaid: 'Register offline payment',
      markFailed: 'Mark order failed'
    },
    processStatus: {
      handled: 'Handled',
      ignored: 'Ignored',
      failed: 'Failed'
    },
    signature: {
      valid: 'Valid',
      invalid: 'Invalid'
    },
    providers: {
      wechat_pay: 'WeChat Pay',
      alipay: 'Alipay',
      offline: 'Offline'
    },
    clients: {
      web: 'Web',
      mobile: 'Mobile',
      admin: 'Admin'
    },
    actions: {
      createPlan: 'Create plan',
      edit: 'Edit',
      enable: 'Enable',
      disable: 'Disable',
      detail: 'Detail',
      markFailed: 'Mark failed',
      createOfflinePayment: 'Register offline payment'
    }
  },
  reports: {
    title: 'Reports',
    subtitle: 'Review learning activity, accuracy, user details, and leaderboards.',
    to: 'to',
    dateFrom: 'Start date',
    dateTo: 'End date',
    userKeyword: 'Search email or nickname',
    trendTitle: 'Daily trend',
    userReportTitle: 'User learning details',
    leaderboardTitle: 'Leaderboards',
    periodTypeFilter: 'Period',
    periodStart: 'Period start',
    metricTypeFilter: 'Metric',
    total: '{total} records',
    unnamed: 'No nickname',
    emptyTrend: 'No trend data',
    emptyUsers: 'No user learning data',
    emptyLeaderboards: 'No leaderboard data',
    minutes: '{value} min',
    seconds: '{value}s',
    hours: '{hours} h {minutes} min',
    summary: {
      activeUsers: 'Active users',
      studyTime: 'Study time',
      exercises: 'Exercises',
      accuracy: 'Accuracy',
      vocabReview: 'Vocab reviews',
      dialogue: 'Dialogue',
      matching: 'Matching'
    },
    chartMetrics: {
      studySeconds: 'Study time',
      exerciseCount: 'Exercises',
      activeUserCount: 'Active users',
      accuracyRate: 'Accuracy'
    },
    periodTypes: {
      daily: 'Daily',
      weekly: 'Weekly',
      monthly: 'Monthly',
      all: 'All-time'
    },
    metricTypes: {
      streak: 'Streak',
      accuracy: 'Accuracy',
      vocab_count: 'Vocab count',
      game_score: 'Game score'
    },
    columns: {
      user: 'User',
      status: 'Status',
      studyTime: 'Study time',
      exerciseCount: 'Exercises',
      correctCount: 'Correct',
      accuracy: 'Accuracy',
      vocabReview: 'Vocab',
      dialogue: 'Dialogue',
      matching: 'Matching',
      lastStudyDate: 'Last study',
      rank: 'Rank',
      metricType: 'Metric',
      score: 'Score',
      period: 'Period',
      generatedAt: 'Generated at'
    },
    actions: {
      search: 'Search',
      reset: 'Reset'
    }
  },
  system: {
    title: 'System',
    subtitle: 'Manage admin roles, runtime configuration, and audit logs.',
    tabs: {
      roles: 'Roles',
      admins: 'Admin accounts',
      configs: 'Configs',
      logs: 'Operation logs'
    },
    roleListTitle: 'Roles',
    adminListTitle: 'Admins',
    permissionEditorTitle: 'Permission editor',
    adminRoleEditorTitle: 'Admin roles',
    selectRoleHint: 'Select a role',
    selectAdminHint: 'Select an admin',
    availablePermissions: 'Available',
    selectedPermissions: 'Selected',
    availableRoles: 'Available roles',
    selectedRoles: 'Selected roles',
    configListTitle: 'System configs',
    logListTitle: 'Operation logs',
    adminKeyword: 'Search username or display name',
    adminStatusFilter: 'Admin status',
    configKeyword: 'Search config key or description',
    configGroupFilter: 'Config group',
    logKeyword: 'Search action, target, or IP',
    actionFilter: 'Action',
    targetTypeFilter: 'Target type',
    createdFrom: 'Start time',
    createdTo: 'End time',
    to: 'to',
    total: '{total} records',
    emptyRoles: 'No roles',
    emptyAdmins: 'No admins',
    emptyConfigs: 'No configs',
    emptyLogs: 'No logs',
    roleDialogCreateTitle: 'Create role',
    roleDialogEditTitle: 'Edit role',
    roleDeleteTitle: 'Delete role',
    roleDeleteConfirm: 'Delete role "{role}"? Its permission settings will also be removed.',
    adminDialogCreateTitle: 'Create admin',
    adminDialogEditTitle: 'Edit admin',
    resetPasswordTitle: 'Reset admin password',
    configDialogTitle: 'Edit system config',
    logDetailTitle: 'Log detail',
    roleCodePlaceholder: 'Enter role code, e.g. content_manager',
    roleNamePlaceholder: 'Enter role name',
    usernamePlaceholder: 'Enter username',
    displayNamePlaceholder: 'Enter display name',
    adminPasswordPlaceholder: 'Enter an 8-72 character password',
    rolesPlaceholder: 'Select roles',
    adminRolesHint: 'Roles decide which admin menus and actions this account can use. A new admin must have at least one role.',
    configValuePlaceholder: 'Enter config value',
    descriptionPlaceholder: 'Enter description',
    saved: 'Saved',
    deleted: 'Deleted',
    requestFailed: 'Operation failed. Please try again later.',
    matchingStages: {
      title: 'Matching difficulty / elimination stage groups',
      subtitle: 'A stage group can contain multiple levels. Level codes create games; elimination displays them as Stage 1, Stage 2, and so on.',
      add: 'Add group',
      save: 'Save stages',
      empty: 'No groups',
      emptyLevels: 'No levels',
      code: 'Code',
      stageCode: 'Group code',
      levelCode: 'Level code',
      levelTitle: 'Level settings',
      addLevel: 'Add level',
      labelZh: 'Chinese label',
      labelEn: 'English label',
      labelRu: 'Russian label',
      pairCount: 'Pairs',
      cardCount: 'Cards',
      timeLimitSeconds: 'Time limit',
      sortOrder: 'Order',
      enabled: 'Enabled',
      validationRequired: 'At least one group is required',
      validationCode: 'Group code can only contain letters, numbers, underscores, and hyphens, up to 30 characters',
      validationDuplicate: 'Group code must be unique',
      validationName: 'Please enter the Chinese group label',
      validationLevelRequired: 'Each group needs at least one level',
      validationLevelCode: 'Level code can only contain letters, numbers, underscores, and hyphens, up to 30 characters',
      validationLevelDuplicate: 'Level code must be unique',
      validationLevelName: 'Please enter the Chinese level label'
    },
    runtime: {
      title: 'Runtime settings',
      subtitle: 'Maintain system parameters customers may adjust in daily operations. Some changes take effect immediately; settings tied to startup wiring or upload gateway limits need a service restart.',
      save: 'Save runtime settings',
      membershipTitle: 'Membership',
      trialDays: 'New user trial days',
      paymentTitle: 'Payment',
      mockPayment: 'Mock payment',
      wechatPay: 'WeChat Pay',
      alipay: 'Alipay',
      wechatPayUrl: 'WeChat payment URL prefix',
      alipayUrl: 'Alipay payment URL prefix',
      secretPlaceholder: 'Leave empty to keep current secret',
      clearSecret: 'Clear current secret',
      secretConfigured: 'Notify secret configured',
      secretNotConfigured: 'Notify secret not configured',
      asrTitle: 'ASR',
      restartHint: 'Changing ASR provider, worker state, or scheduler interval is tied to startup wiring. Restart the service after changing them.',
      asrProvider: 'ASR provider',
      workerEnabled: 'Worker enabled',
      engineName: 'Engine name',
      serviceUrl: 'Service URL',
      servicePath: 'Recognition path',
      timeoutMs: 'Request timeout ms',
      batchSize: 'Batch size',
      initialDelayMs: 'Initial delay ms',
      pollDelayMs: 'Poll interval ms',
      mockRecognizedText: 'Mock recognized text',
      uploadTitle: 'Upload',
      uploadRestartHint: 'Upload size is also constrained by Spring multipart limits and needs a restart. Extension rules take effect for admin uploads and H5 recordings immediately.',
      maxFileSize: 'Max file size',
      maxRequestSize: 'Max request size',
      audioExtensions: 'Audio extensions',
      imageExtensions: 'Image extensions',
      videoExtensions: 'Video extensions'
    },
    adminStatuses: {
      active: 'Active',
      disabled: 'Disabled'
    },
    configGroups: {
      payment: 'Payment',
      asr: 'ASR',
      membership: 'Membership',
      upload: 'Upload',
      learning: 'Learning'
    },
    columns: {
      id: 'ID',
      role: 'Role',
      roleCode: 'Role code',
      roleName: 'Role name',
      adminAccount: 'Admin',
      username: 'Username',
      displayName: 'Display name',
      status: 'Status',
      roles: 'Roles',
      lastLoginAt: 'Last login',
      permissionCount: 'Permissions',
      description: 'Description',
      configKey: 'Config key',
      configGroup: 'Group',
      configValue: 'Value',
      updatedAt: 'Updated at',
      createdAt: 'Created at',
      actions: 'Actions',
      action: 'Action',
      target: 'Target',
      adminUserId: 'Admin ID',
      ipAddress: 'IP address'
    },
    actions: {
      search: 'Search',
      reset: 'Reset',
      edit: 'Edit',
      delete: 'Delete',
      detail: 'Detail',
      cancel: 'Cancel',
      submit: 'Submit',
      createRole: 'Create role',
      createAdmin: 'Create admin',
      saveRoles: 'Save roles',
      resetPassword: 'Reset password',
      savePermissions: 'Save permissions'
    },
    validation: {
      configValueTooLong: 'Config value must be 2000 characters or less',
      roleCodeRequired: 'Enter a role code',
      roleCodePattern: 'Role code must start with a lowercase letter and contain only lowercase letters, numbers, underscores, colons, or hyphens',
      roleCodeTooLong: 'Role code must be 50 characters or less',
      roleNameRequired: 'Enter a role name',
      roleNameTooLong: 'Role name must be 100 characters or less',
      usernameRequired: 'Enter a username',
      usernamePattern: 'Username can contain only letters, numbers, underscores, dots, the at sign, or hyphens',
      usernameTooLong: 'Username must be 100 characters or less',
      displayNameTooLong: 'Display name must be 100 characters or less',
      passwordRequired: 'Enter a password',
      passwordLength: 'Password must be 8-72 characters',
      rolesRequired: 'Select at least one role',
      superAdminRoleProtected: 'The super admin role cannot be deleted',
      statusRequired: 'Select a status',
      descriptionTooLong: 'Description must be 500 characters or less',
      roleDescriptionTooLong: 'Role description must be 2000 characters or less'
    }
  },
  users: {
    title: 'Users',
    subtitle: 'Review student accounts, learning status, account state, and membership access.',
    keywordPlaceholder: 'Search email or nickname',
    statusFilter: 'User status',
    reset: 'Reset',
    listTitle: 'User list',
    total: '{total} records',
    emptyTable: 'No users',
    unnamed: 'No nickname',
    detailTitle: 'User details',
    learningTitle: 'Learning overview',
    membershipRecords: 'Membership records',
    membershipDialogTitle: 'Adjust membership',
    membershipAction: 'Action',
    membershipPlan: 'Plan',
    membershipPeriod: 'Period',
    manualAdjustment: 'Manual adjustment',
    startedAt: 'Start time',
    endsAt: 'End time',
    reason: 'Reason',
    reasonPlaceholder: 'Enter the reason for audit tracking',
    reasonRequired: 'Please enter a reason',
    reasonTooLong: 'Reason must be 1000 characters or less',
    periodRequired: 'Please select the start and end time',
    periodInvalid: 'End time must be later than start time',
    statusDialogTitle: 'Update user status',
    statusReasonPlaceholder: 'Optional reason',
    trialPeriod: 'Trial period',
    cancel: 'Cancel',
    submit: 'Submit',
    saved: 'Saved',
    hours: '{hours} h {minutes} min',
    minutes: '{value} min',
    seconds: '{value}s',
    exerciseSummary: '{count} exercises, {rate} accuracy',
    columns: {
      user: 'User',
      status: 'Status',
      access: 'Access',
      membership: 'Access ends',
      learning: 'Learning',
      createdAt: 'Created at',
      lastLoginAt: 'Last login',
      actions: 'Actions'
    },
    status: {
      active: 'Active',
      disabled: 'Disabled',
      deleted: 'Deleted'
    },
    access: {
      member: 'Member',
      trial: 'Trial',
      free: 'Free'
    },
    actions: {
      detail: 'Detail',
      disable: 'Disable',
      enable: 'Enable',
      membership: 'Membership',
      grantMembership: 'Grant / adjust',
      cancelMembership: 'Cancel membership'
    },
    summary: {
      studyTime: 'Study time',
      exerciseCount: 'Exercises',
      correctCount: 'Correct',
      accuracy: 'Accuracy',
      vocabReview: 'Vocab reviews',
      streak: 'Streak'
    }
  }
}
